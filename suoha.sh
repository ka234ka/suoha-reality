...
persist_keepalive() {
  ...
  # 先确保启动一次（静音 pgrep -f）
  pgrep -f "$BASE_DIR/xray" >/dev/null || nohup "$BASE_DIR/xray" run -config "$CFG_JSON" >>"$LOG_DIR/xray.log" 2>&1 &
  local _ips="${ips:-4}"
  pgrep -f "$BASE_DIR/cloudflared-linux" >/dev/null || nohup "$BASE_DIR/cloudflared-linux" --edge-ip-version "$_ips" --protocol http2 tunnel --config "$CFG_YAML" run >>"$LOG_DIR/cloudflared.log" 2>&1 &
  ...
  while true; do
    ...
    if ! pgrep -f "$BASE_DIR/cloudflared-linux" >/dev/null; then
      echo "[WARN] $(date '+%F %T') Cloudflared 掉线，检测外网..." | tee -a "$LOG_DIR/keepalive.log"
      if curl -m 3 -s https://1.1.1.1 >/dev/null; then
        echo "[INFO] $(date '+%F %T') 外网可用，5 秒后重启 Cloudflared" | tee -a "$LOG_DIR/keepalive.log"
        sleep 5
        ...
        echo "[WARN] $(date '+%F %T') 外网不可用，跳过本轮 Cloudflared 重启" | tee -a "$LOG_DIR/keepalive.log"
      fi
    fi
    sleep 60
  done
...
idxtunnel() {
  ...
  if [ -f "$BASE/config.json" ] && [ -f "$BASE/config.yaml" ] && [ -x "$BASE/xray" ] && [ -x "$BASE/cloudflared-linux" ]; then
    ...
    # 尝试清理残留进程（避免重复）- 静音 pgrep -f
    pgrep -f "$BASE/xray" >/dev/null && pkill -9 -f "$BASE/xray" || true
    pgrep -f "$BASE/cloudflared-linux" >/dev/null && pkill -9 -f "$BASE/cloudflared-linux" || true
